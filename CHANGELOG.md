...
- **Сообщение о таймауте называло число, которое не имело отношения к произошедшему.** Каждый
  бэкенд строит собственный контекст `maxTimeout` внутри своего `Resolve`, поэтому и исчерпанный
  бюджет, и 30-секундный таймаут HTTP-клиента WebDriver приходили в `service.go` одинаково — голым
  `context.DeadlineExceeded`, — а обработчик в обоих случаях печатал `req.MaxTimeout`. Замерено на
  живом сайте: запрос упал за 37.4 с с текстом `Timeout after 220 seconds`, то есть отправлял
  читающего крутить число, которого никто не дожидался. Теперь `resolveTimeoutError` сравнивает
  фактически прошедшее время с бюджетом: если бюджет действительно исчерпан — прежняя,
  wire-совместимая формулировка `Timeout after N seconds.` (клиенты FlareSolverr на неё
  матчатся, она сохранена побайтово); если нет — сообщается реально прошедшее время и то, что
  до `maxTimeout` дело не дошло. Префикс `Error` сохранён, поэтому `prometheusResult` по-прежнему
  считает это ошибкой.
- **Сохранение настроек через web UI стирало комментарии из `init.yaml`.** `SaveConfig` каждый раз
  маршалил структуру `configFile` заново, поэтому один клик «Сохранить» превращал поставляемый
  конфиг с шапкой, описанием каждого значения `browser_backend` и закомментированным блоком `proxy`
  в 11 голых строк. Ключи с пустым значением исчезали заодно — `omitempty` выбрасывал их вместе с
  комментариями над ними. Теперь `renderConfigYAML` правит дерево `yaml.Node` существующего файла
  на месте: комментарии и порядок ключей сохраняются, ключи с пустым значением остаются в файле,
  новые (появившиеся после написания конфига) дописываются, а пустые строки между группами
  восстанавливаются через `HeadComment`, начинающийся с перевода строки, — иначе yaml.v3 схлопывает
  файл в сплошной список. Повреждённый, отсутствующий или неожиданно устроенный файл откатывается
  на обычный маршалинг: сохранение настроек не должно падать из-за битого конфига. Проверено —
  двойное сохранение через `/api/settings` оставляет `init.yaml` побайтово тем же.
